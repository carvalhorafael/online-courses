...
Este repositorio contem o plugin WordPress `online-courses`.

O objetivo do plugin e ser dono do dominio persistente de cursos online:
post type, taxonomia, metadado essencial de checkout e rewrites. Temas devem
consumir esse contrato para renderizacao, mas nao devem registrar novamente
esse dominio.
...
- registrar `course`;
- registrar `course_category`;
- registrar o metadado canonico de checkout;
...
- layout publico;
- templates de tema;
- logica de checkout, pagamento ou matricula;
- area do aluno ou controle de acesso;
...
- Use prefixo interno `online_courses_` para funcoes publicas do plugin e
  `Online_Courses_` para classes.
...
- Internacionalizar textos visiveis com text domain `online-courses`.
...
6. Conferir asset `online-courses-X.Y.Z.zip` na GitHub Release.
